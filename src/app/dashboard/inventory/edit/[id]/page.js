...
import { Package, Sparkles, Image as ImageIcon, Loader2 } from "lucide-react";
...
const inventoryValidationSchema = Yup.object({
  ...
  itemName: Yup.string().required("Item name is required").min(2, "Item name must be at least 2 characters"),
  ...
  sellingPrice: Yup.number()
    .when('itemType', {
      is: 'sale',
      then: (schema) => schema.required('Selling price is required').positive('Must be positive').nullable(),
...
export default function EditInventoryPage() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/inventory/${id}`, {
        ...
        if (data.success) {
          ...
          setInitialValues({
            itemType: item.product.isServiceSupply ? "supply" : "sale",
            itemName: item.product.name,
            category: item.product.category,
            brand: item.product.brand || "",
            unitType: item.product.unitType || "",
            size: item.product.variants?.[0]?.size || "", 
            sellingPrice: item.product.basePrice || "",
  ...
  const handleSubmit = async (values, { setSubmitting, setStatus }) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/inventory/${id}`,
        {
          ...
          body: JSON.stringify({
            ...
            lowStockThreshold: values.minStockAlert ? parseInt(values.minStockAlert) : 10,
          }),
        }
      );
      ...
      console.error(err);
      setStatus({ error: err.message || "Failed to update item." });
  ...
  if (!initialValues) return <div>Item not found</div>;
